...
takes precedence, so the command above still wins — no need to touch the packaged file.

Either way, `hyprctl reload && hyprctl configerrors` afterwards; an empty response is a
clean parse.

### "Reveal in File Explorer" and "Show in folder"

Neither of those goes through the MIME default either. VS Code, Chromium and most GTK and
